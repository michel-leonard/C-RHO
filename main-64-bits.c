...
int main(void) {
    ...
    positive_number *factors = calloc(64, sizeof(positive_number)), n = 0, mask = -1;
    unsigned sr = (size_t)factors; sr = -sr / 561; printf("srand at %u :\n\n", sr); srand(sr);
    for (int wrapper = 4, count = 0, j; wrapper < 64; ++wrapper)
        for (int n_bits = wrapper; n_bits <= 64; ++n_bits) {
            ...
            printf("%5d. (%2d bits) %22llu = ", ++count, n_bits, n);
